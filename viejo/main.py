...
@app.get("/contactos", summary="Endpoint contactos obtener por nombre")
def obtener_contactos(nombre: Optional[str] = None):
    ...
    * 404 Not Found: Si no se encuentran contactos que coincidan con el nombre.
    * 500 Internal Server Error: En caso de un error interno del servidor.
    ...
    if nombre:
        nombre = nombre.lower()
        contactos_encontrados = [contacto for contacto in contactos if nombre in contacto['nombre'].lower()]
    else:
        contactos_encontrados = contactos
    if not contactos_encontrados:
        if nombre:
            raise HTTPException(status_code=404, detail=f"No se encontraron contactos con el nombre que contiene '{nombre}'")
        else:
            raise HTTPException(status_code=404, detail="No se encontraron contactos")
    return contactos_encontrados

@app.post("/contactos", summary="Endpoint contactos crear")
def crear_contacto(contacto: Contacto):
    """
    # Endpoint para crear contactos.
    ## Status Code
    * 201 Created: Si el contacto se crea exitosamente.
    * 500 Internal Server Error: En caso de un error interno del servidor.
    """
    contactos = []
    try:
    ...
    nuevo_id = max([int(c['id_contacto']) for c in contactos], default=0) + 1
    contacto_data = contacto.dict()
    contacto_data['id_contacto'] = str(nuevo_id)
    contactos.append(contacto_data)
    
    try:
        # Intentar escribir los datos del nuevo contacto en el archivo CSV
        with open('contactos.csv', 'w', newline='') as archivo_csv:
            encabezados = contactos[0].keys() if contactos else []
            escritor_csv = csv.DictWriter(archivo_csv, fieldnames=encabezados)
            escritor_csv.writeheader()
            escritor_csv.writerows(contactos)
    except Exception as e:
        # Si hay un error al escribir en el archivo, lanzar una excepción de servidor interno
        raise HTTPException(status_code=500, detail=f"Error en el servidor: {str(e)}")
    
    return contacto_data

@app.put("/contactos", summary="Endpoint contactos editar id_contacto")
def modificar_contacto(id_contacto: int, contacto: Contacto):
    """
    # Endpoint para editar los contactos por su id_contacto.
    ## Status Code
    * 200 OK: Si el contacto se edita exitosamente.
    * 404 Not Found: Si el contacto con el ID especificado no se encuentra.
    * 500 Internal Server Error: En caso de un error interno del servidor.
    """
    contactos = []
    try:
    ...
    encontrado = False
    for c in contactos:
        if c['id_contacto'] == str(id_contacto):
    ...
    if encontrado:
        try:
            # Intentar escribir los datos actualizados en el archivo CSV
            with open('contactos.csv', 'w', newline='') as archivo_csv:
                encabezados = contactos[0].keys() if contactos else []
                escritor_csv = csv.DictWriter(archivo_csv, fieldnames=encabezados)
                escritor_csv.writeheader()
...
@app.delete("/contactos")
def eliminar_contacto(id_contacto: int):
    """
    # Endpoint para eliminar los contactos por su id_contacto.
    
    ## Status Code
    * 204 No Content: Si el contacto se elimina exitosamente.
    * 404 Not Found: Si el contacto con el ID especificado no se encuentra.
    * 500 Internal Server Error: En caso de un error interno del servidor.
    
    :param id_contacto: El ID del contacto que se va a eliminar.
    :return: Un diccionario con un mensaje indicando el resultado de la operación.
    """
    contactos = []
    try:
        with open('contactos.csv', 'r', newline='') as archivo_csv:
        ...
        contactos = [c for c in contactos if c['id_contacto'] != str(id_contacto)]
        ...
        return {"mensaje": f"Contacto con ID {id_contacto} eliminado exitosamente"}, 204
    except FileNotFoundError:
        return {"mensaje": f"Contacto con ID {id_contacto} no encontrado"}, 404
    except Exception as e:
...
@app.post("/imagenes", status_code=201, summary="Endpoint para subir imágenes")
async def crear_imagen(
    imagenes: list[UploadFile] = File(...),
    cortar: str = Query(None),
    colorize: bool = Query(False),
    fliph: bool = Query(False)
):
    """
    # Endpoint para subir imágenes
    ## Status Code
    * 201 Created: Imágenes subidas con éxito.
    * 400 Bad Request: Si hay un error en la solicitud.
    ...
    for archivo in imagenes:
        try:
            ...
            if colorize:
                # Apply colorize (change the colors of the image)
                # Assuming colorize_color is the desired color
                colorize_color = (255, 0, 0)  # Red
                image = ImageOps.colorize(image.convert("L"), black="#000000", white=colorize_color)

            if fliph:
                # Apply fliph (horizontal flip)
    ...
    return {"message": f"Archivos subidos en http://localhost:8000/static/images/{archivo.filename}"}
...
async def main():
    ...
    <body>
        ...
        <form action="/imagenes" method="post" enctype="multipart/form-data">
            ...
            <input type="checkbox" id="colorize" name="colorize" value="true">
            <br>
            <label for="fliph">Flip horizontal:</label>
            <input type="checkbox" id="fliph" name="fliph" value="true">
            <br>
